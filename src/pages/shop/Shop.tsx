...
const Shop = () => {
    ...
    useMemo(() => {
        ...
        const productStore = products.filter((product) => product.storeId === Number(id))
        setStore(storeData)
                    ...
                    <div className='flex flex-col items-center w-100% md:flex-row md:gap-x-8'>
                        <div className='space-y-8 md:w-3/5'>
                            ...
                            <div>
                                ...
                                <div>Website: <u className='text-link cursor-pointer'>{store?.website}</u></div>
                                <div>Email: <u className='text-link cursor-pointer'>{store?.email}</u></div>
                                <div>Giờ mở cửa: {store?.openHours && store?.openHours[0]?.startTime && (`${store.openHours[0]?.startTime} - ${store.openHours[0]?.endTime}`)}</div>
                            </div>
                        </div>
                        <div className='flex flex-col items-center w-100% h-[448px] overflow-hidden md:w-2/5 md:flex-row mt-[1rem] rounded-xl'>
                            <img src={store?.imgUrl} alt="" className='object-cover h-full'/>
                        </div>
                    </div>
                </div>
                <StyleDivider />
                <div className='py-8 text-[32px] flex flex-col justify-center items-center'>
                    <div className='text-normal text-center uppercase'>Sản phẩm của {store?.title}</div>
